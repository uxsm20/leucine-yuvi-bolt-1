...
const CreateRoleModal: React.FC<CreateRoleModalProps> = ({
  ...
  const toggleAllEntityPermissions = (entityId: string, checked: boolean) => {
    const entityPermissions = permissions.filter(p => p.entityId === entityId);
    const newPermissions = new Set(selectedPermissions);
  ...
  const toggleAllPagePermissions = (pageId: string, checked: boolean) => {
    const pagePermissions = permissions.filter(p => p.pageId === pageId);
    const newPermissions = new Set(selectedPermissions);
  ...
  const areAllEntityPermissionsSelected = (entityId: string) => {
    const entityPermissions = permissions.filter(p => p.entityId === entityId);
    return entityPermissions.every(permission => selectedPermissions.has(permission.id));
  };

  const areAllPagePermissionsSelected = (pageId: string) => {
    const pagePermissions = permissions.filter(p => p.pageId === pageId);
    return pagePermissions.every(permission => selectedPermissions.has(permission.id));
  };
                                  ...
                                  .map(entity => (
                                    <div key={entity.id} className="bg-gray-900 rounded-lg p-4">
                                      <div className="flex items-center justify-between mb-4">
                                        <h6 className="text-sm font-medium text-white">{entity.name}</h6>
                                        <label className="flex items-center space-x-2">
                                          <input
                                            type="checkbox"
                                            checked={areAllEntityPermissionsSelected(entity.id)}
                                            onChange={(e) => toggleAllEntityPermissions(entity.id, e.target.checked)}
                                      ...
                                      <div className="grid grid-cols-2 gap-4">
                                        {['create', 'read', 'update', 'delete'].map(actionType => {
                                          const permission = getEntityPermissions(entity.id)
                                            .find(p => p.actionType === actionType);
                                          ...
                                          return (
                                            <label
                                              key={permission.id}
                                              className="flex items-center space-x-2 text-sm text-gray-300 bg-gray-800 p-2 rounded-lg"
                                            >
                                              <input
                                                type="checkbox"
                                ...
                                {pages
                                  ...
                                  .map(page => (
                                    <div key={page.id} className="bg-gray-900 rounded-lg p-4">
                                      <div className="flex items-center justify-between mb-4">
                                        <h6 className="text-sm font-medium text-white">{page.name}</h6>
                                        <label className="flex items-center space-x-2">
                                          <input
                                            type="checkbox"
                                            checked={areAllPagePermissionsSelected(page.id)}
                                            onChange={(e) => toggleAllPagePermissions(page.id, e.target.checked)}
                                            className="rounded border-gray-700 bg-gray-900 text-indigo-500 focus:ring-indigo-500"
                                          />
                                          <span className="text-sm text-gray-300">Select All</span>
                                        </label>
